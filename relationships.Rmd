...
joined_data %>%
    ...
    geom_jitter(size=.4, alpha=0.1, width=0.1, height=0.5) +
    theme_tufte() + coord_cartesian(xlim=c(-8,8), ylim=c(0,200)) +facet_wrap(~ locat, ncol=2)
    
```



Are those in opposition real people 

```{r}
joined_data %>% filter(locat == "NZ") %>% arrange(balance) %>% select(screen_name.x, name, description, balance) %>%
    distinct() %>% View()
```
